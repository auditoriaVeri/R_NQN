  ...
  if ($respVer) {
    ...
    foreach ($respVer as $row) {
      ...
      if ($respServ = $dbUNC->query($sqlSer)) {
        ...
        if ($row["NroDocConductor"] == "")
          $row["NroDocConductor"] = 'NULL';
        if ($row["Eje1_Tara"] == "")
          $row["Eje1_Tara"] = 'NULL';
        if ($row["Eje2_Tara"] == "")
          $row["Eje2_Tara"] = 'NULL';
        ...
        if ($row["Eje5_Tara"] == "")
          $row["Eje5_Tara"] = 'NULL';
        if ($row["Eje1_FzaIzq"] == "")
          $row["Eje1_FzaIzq"] = 'NULL';
        if ($row["Eje2_FzaIzq"] == "")
          $row["Eje2_FzaIzq"] = 'NULL';
        if ($row["Eje3_FzaIzq"] == "")
          $row["Eje3_FzaIzq"] = 'NULL';
        if ($row["Eje4_FzaIzq"] == "")
          $row["Eje4_FzaIzq"] = 'NULL';
        if ($row["Eje5_FzaIzq"] == "")
          $row["Eje5_FzaIzq"] = 'NULL';
        if ($row["Eje1_FzaDer"] == "")
          $row["Eje1_FzaDer"] = 'NULL';
        if ($row["Eje2_FzaDer"] == "")
          $row["Eje2_FzaDer"] = 'NULL';
        if ($row["Eje3_FzaDer"] == "")
          $row["Eje3_FzaDer"] = 'NULL';
        if ($row["Eje4_FzaDer"] == "")
          $row["Eje4_FzaDer"] = 'NULL';
        if ($row["Eje5_FzaDer"] == "")
          $row["Eje5_FzaDer"] = 'NULL';
        if ($row["Eje1_Dif"] == "")
          $row["Eje1_Dif"] = 'NULL';
        if ($row["Eje2_Dif"] == "")
          $row["Eje2_Dif"] = 'NULL';
        ...
        if ($row["Eje5_Dif"] == "")
          $row["Eje5_Dif"] = 'NULL';
        if ($row["Eje1_Eficiencia"] == "")
          $row["Eje1_Eficiencia"] = 'NULL';
        if ($row["Eje2_Eficiencia"] == "")
          $row["Eje2_Eficiencia"] = 'NULL';
        if ($row["Eje3_Eficiencia"] == "")
          $row["Eje3_Eficiencia"] = 'NULL';
        if ($row["Eje4_Eficiencia"] == "")
          $row["Eje4_Eficiencia"] = 'NULL';
        if ($row["Eje5_Eficiencia"] == "")
          $row["Eje5_Eficiencia"] = 'NULL';
        if ($row["Interior"] == "")
          $row["Interior"] = 'NULL';
        if ($row["Escape"] == "")
          $row["Escape"] = 'NULL';
        if ($row["Bach"] == "")
          $row["Bach"] = 'NULL';
        if ($row["PorcentajeCo"] == "")
          $row["PorcentajeCo"] = 'NULL';
        if ($row["Opac"] == "")
          $row["Opac"] = 'NULL';
        if ($row["ppmHC"] == "")
          $row["ppmHC"] = 'NULL';
        if ($row["Freno_FzaIsq"] == "")
          $row["Freno_FzaIsq"] = 'NULL';
        if ($row["Freno_FzaDer"] == "")
          $row["Freno_FzaDer"] = 'NULL';
        if ($row["Freno_Dif"] == "")
          $row["Freno_Dif"] = 'NULL';
        if ($row["Freno_Eficiencia"] == "")
          $row["Freno_Eficiencia"] = 'NULL';
        if ($row["Susp_FzaIsq"] == "")
          $row["Susp_FzaIsq"] = 'NULL';
        if ($row["Susp_FzaDer"] == "")
          $row["Susp_FzaDer"] = 'NULL';
        if ($row["Susp_Dif"] == "")
          $row["Susp_Dif"] = 'NULL';
        if ($row["Susp_Eficiencia"] == "")
          $row["Susp_Eficiencia"] = 'NULL';
        if ($row["MotorAnio"] == "")
          $row["MotorAnio"] = 'NULL';
        if ($row["VAnio"] == "")
          $row["VAnio"] = 'NULL';
        ...
        if ($row["Asientos"] == "")
          $row["Asientos"] = 'NULL';
        if ($row["idLocTMServ"] == "")
          $row["idLocTMServ"] = 'NULL';
        if ($row["idTipoServicio"] == "")
          $row["idTipoServicio"] = 'NULL';
        if ($row["idClaseServicio"] == "")
          $row["idClaseServicio"] = 'NULL';
        if ($row["PNroDoc"] == "")
          $row["PNroDoc"] = 'NULL';
        if ($row["PidLocalidad"] == "")
          $row["PidLocalidad"] = 'NULL';
        if ($row["NroInterno"] == "")
          $row["NroInterno"] = 'NULL';
        if ($row["idTipoUso"] == "")
          $row["idTipoUso"] = 'NULL';
        if ($row["idVerificacionOriginal"] == "")
          $row["idVerificacionOriginal"] = 'NULL';
        ...
        if ($respServ->rowCount() > 0) {
          ...
          $sqlInSer = "INSERT INTO verificaciones(idVerificacion,idTaller,Fecha,Hora,DominioVehiculo,ChasisNro,MotorNumero,CodigoTitular,DescripcionTitular,
                    ...
                    PTipoDoc,PTipoPersona,PNroDoc,PCuit,PDomicilio,PTelefono,PEmail,PidLocalidad,usuarioCarga,TipoCarga,
                    idVerificacionOriginal,Reverificacion,PCodigoPJ) 
                    VALUES 
                    (" . $row["idVerificacion"] . "," . $row["idTaller"] . ",'" . $row["Fecha"] . "','" . $row["Hora"] . "','" . $row["DominioVehiculo"] . "',
                      '" . addslashes($row["ChasisNro"]) . "','" . addslashes($row["MotorNumero"]) . "','" . $row["CodigoTitular"] . "','" . addslashes($row["DescripcionTitular"]) . "',
              ...
              "','" . addslashes($row["CompaniaSeguro"]) . "','" . addslashes($row["NroPoliza"]) . "','" . addslashes($row["UltimoRecPatente"]) . "'," . $row["MotorAnio"] . ",'" . $row["MotorMarca"] .
              "'," . $row["idLocalidadVehiculo"] . ",'" . addslashes($row["MarcaTac"]) . "','" . $row["NroTac"] . "','" . $row["NroInterno"] . "'," . $row["idTipoUso"] . "," .
              $row["Eje5_Tara"] . "," . $row["Eje5_FzaIzq"] . "," . $row["Eje5_FzaDer"] . "," . $row["Eje5_Dif"] . "," . $row["Eje5_Eficiencia"] . "," . $row["idTipoVehiculo"] . ",'" .
              addslashes($row["VMarca"]) . "','" . addslashes($row["VModelo"]) . "','" . $row["VAnio"] . "','" . addslashes($row["ChasisMarca"]) . "','" . $row["ChasisAnio"] . "','" . addslashes($row["TipoCombustible"]) . "','" . $row["VPotencia"] .
              "'," . $row["NroEjes"] . ",'" . $row["VCaja"] . "','" . addslashes($row["PocisionMotor"]) . "','" . addslashes($row["Carroceria"]) . "','" . addslashes($row["Expediente"]) . "'," . $row["AireAco"] . ",
              ...
              "'," . $row["PidLocalidad"] . ",'" . $row["usuarioCarga"] . "','" . addslashes($row["TipoCarga"]) . "'," . $row["idVerificacionOriginal"] . "," . $row["Reverificacion"] . ",'" . $row["PCodigoPJ"] . "')";
        }
        //echo $sqlInSer."<br />";
...
// Defectos de las verificaciones
if ($continua)
{
  $sql= "SELECT * FROM verificacionesdefectos WHERE Replicado = 0";
  $vd= $base->query($sql);
  if ($vd)
  {
    foreach ($vd as $row)
    {
      $sqlInSer= 
        ...
        (
          idVerificacion, idTaller, idDefecto, idNivel, FechaHoraRep, ,
        )
        VALUES
        (
          " . $row['idVerificacion'] . "," . $row['idTaller'] . "," . $row['idDefecto'] . "," . $row['idNivel'] .
        ")";
      
      // Insert en la base en la nube
      if($dbUNC->query($sqlInSer))
      {		
        /**** una vez que cargue en server, le indico en el taller que ya fue replicado ****/		
        $sql = 
          "UPDATE verificacionesdefectos "
          . "SET Replicado = 1 "
          . "WHERE "
          . "idVerificacion = ".$row["idVerificac"]." AND "
          . "idTaller = ".$row["idTaller"] . "AND"
          . "idDefecto = " .$row["idDefecto"];
                
				if(!$base->query($sql))
        {
          /**** SI OCURRIO UN ERROR CORTAMOS EL PROCESO *****/
					$msnlog = "Ocurrio un al marcar como replicado un defecto en un taller. - Taller: $nomTaller";
					$continua = false;
					break;
				}
        else
        {
          $cantDefectosEnVerificaciones++;
				}
			}
			else
      {
        /**** SI OCURRIO UN ERROR CORTAMOS EL PROCESO *****/
        $msnlog = "Ocurrio un error al cargar los defectos de las verificaciones en central. Taller: $nomTaller";
        $continua = false;
        break;
			}		
    }
...
if ($continua) {
  ...
  if ($respCert) {
    ...
    foreach ($respCert as $row) {
      ...
      if ($respServ = $dbUNC->query($sqlSer)) {
      if ($row["porcentajeCategoria"] == "")
        $row["porcentajeCategoria"] = 'NULL';
      if ($row["idCategoria"] == "")
        ...
        if ($respServ->rowCount() > 0) {
          ...
          $sqlInSer = "INSERT INTO certificados(idCertificado,idTaller,NroCertificado,Fecha,Hora,idEstado,VigenciaHasta,
										idVerificacion,idCategoria,porcentajeCategoria,Anulado,FechaAnulacion,Observaciones,Auditoria,Reverificado,Vencido,FechaHoraRep) 
										VALUES 
	      ...
	      '" . $row["Observaciones"] . "','" . $row["Auditoria"] . "'," . $row["Reverificado"] . "," . $row["Vencido"] . ",NOW())";
        }
        //echo $sqlInSer;
        if ($dbUNC->query($sqlInSer)) {
          /*           * ** una vez que cargue en server, le indico en el taller que ya fue replicado *** */
